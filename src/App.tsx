...
export default function App() {
  ...
  const [categories, setCategories] = useState<VendorCategory[]>(INITIAL_VENDOR_CATEGORIES);
  ...
  const [addonOrders, setAddonOrders] = useState<ExhibitorAddonOrder[]>(INITIAL_ADDON_ORDERS);
  const [paymentTransactions, setPaymentTransactions] = useState<ExhibitorPaymentTransaction[]>(INITIAL_PAYMENT_TRANSACTIONS);
  const [serviceTickets, setServiceTickets] = useState<ExhibitorServiceTicket[]>(INITIAL_EXHIBITOR_TICKETS);

  // Domain Core Data
  const [expos] = useState(EXPOS_DATA);
  const [rfqs, setRfqs] = useState<RFQRequest[]>(INITIAL_RFQS);
  const [exhibitors, setExhibitors] = useState<ExhibitorAccount[]>(INITIAL_EXHIBITOR_ACCOUNTS);
  const [threeWayMatches, setThreeWayMatches] = useState<ThreeWayMatchItem[]>(INITIAL_THREE_WAY_MATCHING);
  const [tasks, setTasks] = useState<TaskCardType[]>(INITIAL_TASKS);
  const [booths, setBooths] = useState<BoothItem[]>(INITIAL_BOOTHS);
